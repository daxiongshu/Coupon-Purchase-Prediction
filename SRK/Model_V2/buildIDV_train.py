...
if __name__ == "__main__":
	...
	out_file = csv.writer(open("train_idvs_seed1234.csv","w"))
	random.seed(1234)
	user_count = 0
	out_row_count = 0
	for user_id in coupon_purchase_detail_dict:
		...
		for ind, coupon_dict in enumerate(coupon_purchase_detail_userid_list):
			...
			out_row = [user_id, purchase_coupon] + user_features + user_hist_features + user_visit_features + coupon_features + [dv_value]
			...
			if len(daywise_coupon_list) > no_of_non_purchased_coupons :
				...
				for non_purchased_coupon in non_purchased_coupons:
					...
					dv_value = 0
					out_row = [user_id, non_purchased_coupon] + user_features + user_hist_features + user_visit_features + coupon_features + [dv_value]
		                        assert len(out_row) == out_header_len
                		        out_file.writerow( out_row )
		                        out_row_count += 1
